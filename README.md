...
## HOW IT WORKS
- The repo has the "exercises" folder with 3 sub-folders (part1, part2, and part3)
- Every sub-folder contains exercise files (e1.js, e2.js, ... etc). Each file includes the task description, a function declared, data imported if required, and a placeholder for your code.
- Open e1.js file and start writing functions' code in the "Your code goes here..." placeholder to achieve the goals described.
- **!!! You can use FOR LOOPS ONLY to iterate through data array/nested arrays. Other loops are NOT allowed.**
- Once you are done, run the test command ```npm run test-part1-1``` in the command line, where "1" is the current exercise number of the current part. This will start the test environment and show the passed and failed tests for the current code you have written.
